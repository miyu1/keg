...
Class _$UserHelper and _$AppDatabase are generated by keg_generator.
...
Full list of generated methods are described later.
...
Packages you need is, keg_annotation, keg_generator, [build_runner], and 
[sqflite] (or [sqflite_common_ffi]).
...
Note back link is to acquire by query methods. registerCategory do nothing on itemList.
...
Again, back link is for query methods, so registerItem do nothing on orderList.  
...
`registerUser` throws StateError if id is 0.  
...
```
This sample makes 3 index, user_name_idx, user_updated_idx and user_contact_idx.
...
Sample with[json_serializable] and [freezed].
```dart
@table
@freezed
@JsonSerializable()
...
When use with [freezed], it is not able to set `id` value automatically.  
...
Open or create in memory SQLite file.
...
keg_generator does not support downgrade, UnimplementedError occurs
...
`Future<T> transaction<T>(Future<T> Function(_$AppDatabaseTransactionWrapper txn) action,
 {bool? exclusive,})`
...
If you need to see full code of these samples check [keg_tenerator test](https://github.com/miyu1/keg/tree/main/keg_generator/test) codes.
