...
class Item  with _$Item {
  ...
  const Item(this.name, {this.category, this.id = 0});
...
void main() async {
  ...
  test('freezed test', () async {
    var cat = Category('pen');
    final catId = await appdb.registerCategory(cat);
    expect(cat.id, 0);
    expect(catId, greaterThan(0));
    cat = cat.copyWith(id: catId);
    ...
    expect(json['itemList'].length, 1);
    expect(json['itemList'][0]['id'], item.id);
    expect(json['itemList'][0]['name'], item.name);
    
    final cat2 = Category.fromJson(json);
    expect(cat2.name, cat.name);
    expect(cat2.itemList.length, 1);
    expect(cat2.itemList[0].id, item.id);
    expect(cat2.itemList[0].name, item.name);
