...
Future<List<String>> testGenerator(String name) async {
  ...
  final sourceAssets = <String, Object>{'a|$name.dart': src};
  final dataFile2 = File('test/$name.g.dart');
  if (dataFile2.existsSync()) {
    final src2 = await dataFile2.readAsString();
    sourceAssets['a|$name.g.dart'] = src2;
  }
  ...
  ProcessRunner._(this.executable, this.arguments, this.process) {
    ...
    stdoutStream.listen((value) {
      ...
      if (value.isNotEmpty) {
        ...
        values = values.map((e) => e.trim()).toList();
